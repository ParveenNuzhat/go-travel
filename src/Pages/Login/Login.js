...
const Login = () => {
  const { signInUsingGoogle, handleUserLogin, setUser, setIsLoading } =
    useAuth();
    ...
    <div className="m-5">
      ...
      >
        ...
        <div className="text-center">
          <Button className="login-btn w-75" onClick={handleGoogleLogin}>
            LOG IN WITH GOOGLE
          </Button>
        </div>
        <br />
        <p className="mt-3 text-center">
          New to Royal Motor World? <Link to="/register">Sign up now!</Link>
        </p>
      </div>
    </div>
  );
